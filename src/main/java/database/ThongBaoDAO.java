...
public class ThongBaoDAO implements DAOInterface<NoiDungThongBao> {
	...
	public NoiDungThongBao selectByID(String t) {
		...
		try {
			...
			while (rs.next()) {
				...
				result = new NoiDungThongBao(tieuDe,noiDungThongBao, ngayThongBao, loaiThongBao);
				break;
			}
	...
	public int update(NoiDungThongBao t) {
		...
		try {
			...
			st.setString(1, t.getTieuDe());
			st.setString(2, t.getNoiDung());
			st.setDate(3, (java.sql.Date)(t.getThoiGianThongBao()));
			st.setString(4, String.valueOf(t.getLtb()));
	...
	public static void main(String[] args) {
		...
		//LoaiThongBao ltb=new LoaiThongBao();
		System.out.println(tbd.selectAll().toString());
		//System.out.println(tbd.selectByType("TBNB").toString());

		
//		ltb.setMaLoaiThongBao("TBC");
		//Date now = new Date();
		//NoiDungThongBao ndtb=new NoiDungThongBao("insert nd tb",now,ltb);
//		tbd.insert(ndtb);
