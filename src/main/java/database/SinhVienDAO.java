...
public class SinhVienDAO implements DAOInterface<SinhVien> {
	...
	public SinhVien selectByID(String t) {
		...
		try {
			...
			while (rs.next()) {
				...
				result = new SinhVien(maSinhVien, hoTen, ngaySinh, gioiTinh, soDienThoai, CCCD, email,
						tamTru, lop, queQuan, taiKhoan);
				break;
			}
	...
	public int update(SinhVien t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "UPDATE SinhVien " + "SET " + "HoTen = ?," + "NgaySinh = ?," + "GioiTinh = ?,"
					+ "SDT = ?," + "CCCD = ?," + "Email = ?," + "TamTru = ?," + "MaLop = ?," + "MaTinh = ?," + "UserName = ?" + "WHERE MaSinhVien = ?";
